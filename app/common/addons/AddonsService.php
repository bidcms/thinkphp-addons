...
class AddonsService extends \think\Service
{
    ...
    {
        include_once $this->app->getRootPath() . "app/common/addons/helper.php";
    ...
    public function boot()
    {
        $this->registerRoutes(function (Route $route) {
            // 路由脚本
            $execute = '\\app\\common\\addons\\Route::execute';
    ...
    private function autoload()
    {
        ...
        // 读取插件目录及钩子列表
        $base = get_class_methods("\\app\\common\\addons\\Addons");
        // 读取插件目录中的php文件
            ...
            if (strtolower($info['filename']) === $name) {
                ...
                foreach ($hooks as $hook) {
                    ...
                    if (!in_array($name, $config['hooks'][$hook])) {
                        if (is_file($info['dirname'] . '/install.lock')) {
